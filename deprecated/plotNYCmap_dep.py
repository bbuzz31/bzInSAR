...
## for inset plots
from VLM.bzFRInGE.plotting.plotTS import *

DCT_TRANSECTS = {'NYC': 'NJ Manhattan Brooklyn'.split()}

## plot parms
class PlotBase(ExpBase):
    def __init__(self, dct_exp, mp_exp='Base', ref_sta=None, neofs=''):
        super().__init__(dct_exp, mp_exp, ref_sta, neofs, True)

        self.df      = prep_gps(self.path_gps, self.reg, units='mm')
        ...
        self.pparms  = {'transform': self.proj, 'alpha':1.0, 'shading': 'nearest'}
        self.zoom    = 13


    def set_kind(self, kind='rate'):
        self.kind = kind.title()
        assert self.kind in 'Rate Unc'.split(), f'Invalid kind: {kind}'

        if self.kind == 'Rate':
            self.pparms = {**self.pparms, 'cmap': 'cmc.vik',
                           'norm': mpl.colors.TwoSlopeNorm(0, -5, 5)}

            # self.cbar_lbl = 'V$_{Up}$ (mm/yr)'
            self.cbar_lbl = 'Vertical Rate (mm/yr)'
        elif self.kind == 'Unc':
            self.pparms = {**self.pparms, 'cmap': 'cmc.lajolla',
...
def plot_basic(PObj, kind='rate', show_gps=False, use_stitched=False, axes=None):
    """ Plot rate or uncertainty in a single panel w/wout GPS """
    PObj.set_kind(kind)
    if use_stitched:
        path = PObj.path_rate_msk_nc_stitch if PObj.kind == 'Rate' else PObj.path_std_msk_nc_stitch
    else:
        path = PObj.path_rate_msk_nc if PObj.kind == 'Rate' else PObj.path_std_msk_nc
        # path = PObj.path_rate_nc if PObj.kind == 'Rate' else PObj.path_std_nc
    ds = xr.open_dataset(path)
    da = ds['Band1'].rename(PObj.kind)
    da = da*1000 if 'm' in da.units else da

    ## cropping the plot
    # da_reg = da.sel(lat=slice(PObj.SNWE[0], PObj.SNWE[1]),
    #                 lon=slice(PObj.SNWE[2], PObj.SNWE[3]))
    da_reg   = da
    ...
    if interactive:
        ...
        da_reg1 = da_reg.sel(lon=slice(-73.88460830568238, -73.8486756943176),
                             lat=slice(40.75743769431761, 40.79337030568239))
        plot = da_reg1.hvplot.quadmesh(*f'lon lat {PObj.kind}'.split(),
                coastline='10m', geo=True,  cmap='coolwarm', clim=(-5, 5))
        hvplot.show(plot)

    if axes is None:
        fig, axes = plt.subplots(figsize=(10,10), subplot_kw={'projection': PObj.basemap.crs})
        axes.set_extent(PObj.WESN, crs=PObj.proj)
        gl = axes.gridlines(draw_labels=True)
        bbPlot.fmt_gridlines(gl, bottom=True)
    else:
        fig = axes.get_figure()

    axes.add_image(PObj.basemap, PObj.zoom)

    ## add the interpolated GPS rates
    im   = axes.pcolormesh(da_reg.lon, da_reg.lat, da_reg, **PObj.pparms)

    bbPlot.cartopy_cbar(im, ylabel=PObj.cbar_lbl)
    fig.set_label(f'Vup_{PObj.reg}_{PObj.kind}')

    ## add the GPS
    if show_gps:
        plot_gps(PObj, PObj.kind, axes)


    # print(f'Vup {PObj.kind}: {da.mean().item():.2f} +/- {da.std().item():.2f} mm/yr')
    l, h, mu = da_reg.min().item(), da_reg.max().item(), da_reg.mean().item()
    print (f'Vup {PObj.kind}: {mu:.2f} [{l:.2f}, {h:.2f}]')

    return fig, axes


def plot_basic_box(PObj, pts='LaGuardia Williams'.split(), kind='rate', use_stitched=False):
    """ Plot basic plus a 2000 m bbox around point to show inset locations """
    fig, axes  = plot_basic(PObj, kind, True, use_stitched)
    ...
    for pt in pts:
        ...
        axes.add_geometries([bbox], crs=PObj.proj, **sty)
    fig.set_label(f'{fig.get_label()}_{"_".join(pts)}')
    return


def plot_gps(PObj, kind='rate', axes=None):
    """ Plot the MIDAS GPS locations (+ rates and names if axes=None) """
    PObj.set_kind(kind)
    col  = 'u_vel' if PObj.kind == 'Rate' else 'u_sig'
    msty = dict(marker='o', s=50, linewidth=1.5,
                zorder=30, edgecolors='black', c=PObj.df[col],
                **PObj.pparms)
    del msty['shading']
    ...
    if axes is None:
        fig, axes = plt.subplots(figsize=(10,10), subplot_kw={'projection': PObj.basemap.crs})
        axes.add_image(PObj.basemap, PObj.zoom)
        s = axes.scatter(PObj.df['lon'], PObj.df['lat'], **msty)

        bbPlot.cartopy_cbar(s, 'V$_{up}$ (mm/yr)')
        axes.set_title(f'MIDAS GPS {PObj.reg}')
        fig.set_label(f'Midas_{PObj.reg}')
    ...
    ref_sta = PObj.df[PObj.df.sta == PObj.ref_sta]
    msty    = {**msty, 'marker':'s', 'c':ref_sta['u_vel']}
    ...
    if PObj.kind == 'Rate':
        ...
        names = PObj.df['sta'].astype('str').values
        for i in range(len(names)):
            ...
            axes.text(PObj.df['lon'].iloc[i], PObj.df['lat'].iloc[i],
                    names[i].strip(), transform=text_transform, zorder=20,
                    verticalalignment='center', horizontalalignment='right',
                    bbox=sty_name)

    bbLogger.logger.info(f'Raw GPS mean: {PObj.df.u_vel.mean():.2f} +/- {PObj.df.u_sig.mean():.2f} mm/yr')
...
def plot_transect(PObj, use_stitched=False):
    """ Plot the Transects on the rate map with the GPS """
    import geopandas as gpd
    import string
    fig, axes  = plot_basic(PObj, 'rate', True, use_stitched)
    for i, transect in enumerate(DCT_TRANSECTS[PObj.reg]):
        path = op.join(PObj.path_wd, 'Transects', f'Transect_{transect}.GeoJSON')
        gdf = gpd.read_file(path)
        gdf.plot(ax=axes, transform=PObj.proj, color='w')
...
def plot_hist(PObj, kind='rate', use_stitched=False):
    ...
    bbPlot.plot_hist(da_reg.data.reshape(-1))
    return


def plot_insets_col(PObj, pts='LaGuardia Williams'.split(), kind='rate', use_stitched=False):
    ...
    axeMap0 = plt.subplot(gs[0, 0], projection=PObj.basemap.crs)
    axeMap1 = plt.subplot(gs[0, 1], projection=PObj.basemap.crs)
    ...
    pts = 'LaGuardia', 'Williams'
    for i, pt in enumerate(pts):
        lalo = dct_pts[pt][0] #  use plot_ts to make a box around the point
        S, N, W, E = buffer_point(*lalo, 2000)

        PObj.zoom  = 15
        axes  = plot_basic(PObj, kind, False, use_stitched, axes=axeMaps[i])[1]
        ...
        axeTS = plot_ts_vup1_dd(PObj, pts[i], f'{pts[i]}_Stable', npix=0, axes=axeTSs[i])[1]
        axeTS.set_title('')
        if i == 1:
            axeTS.set_ylabel('')
        else:
...
def plot_inset(PObj, pt='LaGuardia', kind='rate', use_stitched=False):
    ...
    ax  = plot_basic(PObj, kind, False, use_stitched, axes=axes[0])[1]
    ...
    axeTS = plot_ts_vup1_dd(PObj, pt, f'{pt}_Stable', npix=0, axes=axes[1])[1]
...
def plot_insets(PObj, pts='LaGuardia Williams'.split(), kind='rate', use_stitched=False):
    ...
    axeMaps = []
    axeTSs  = []
    axeMaps.append(plt.subplot(gs[0, 0], projection=PObj.basemap.crs))
    axeMaps.append(plt.subplot(gs[1, 0], projection=PObj.basemap.crs))
    ...
    for i, pt in enumerate(pts):
        ...
        ax  = plot_basic(PObj, kind, False, use_stitched, axes=axeMaps[i])[1]
        im  = ax.get_children()[0]
        im.set_alpha(0.75)
        im.colorbar.remove()
        ax.set_extent([W, E, S, N])
        ...
        axeTS = plot_ts_vup1_dd(PObj, pt, f'{pt}_Stable', npix=npix, axes=axeTSs[i])[1]
        # axeTSs[i].plot(np.random.rand(180))
        axeTS.set_title('')
    ...
    cbar_ax = fig.add_axes([0.0575, 0.490, 0.21, 0.0175]) # left, bot, wid, hgt
...
def plot_cities(PObj, kind='Rate', city='NJ'):
    """ Plot the rates with city boundaries and calculate the mean

    Manhattan, Staten Island, NJ
    """
    ...
    PObj.set_kind(kind)
    path = PObj.path_rate_msk_nc if PObj.kind == 'Rate' else PObj.path_std_msk_nc
    da   = xr.open_dataarray(path).rename(PObj.kind)
    da = da*1000 if 'm' in da.units else da
    ...
    l, h, mu = da_reg.min().item(), da_reg.max().item(), da_reg.mean().item()
    bbLogger.logger.info (f'{city} Vup {PObj.kind}: {mu:.1f} [{l:.1f}, {h:.1f}]')



## ----------- Wrappers
def make_main(Exp1):
    plot_basic_box(Exp1, pts='LaGuardia Williams'.split())
    # plot_insets(Exp1, pts='LaGuardia Williams'.split())


def make_supp(Exp1):
    plot_basic_box(Exp1, 'Ashe Woodside'.split(), kind='Unc')
    # plot_insets(Exp1, 'Ashe Woodside'.split())
    plot_transect(Exp1)


if __name__ == '__main__':
    Exp       = PlotBase(NYC_SRc, 'ERA5_SET_PM_Fast2017', 'NYBK', neofs=20)

    # make_main(Exp)
    # make_supp(Exp)


    # plot_basic(Exp, 'Rate', show_gps=True)
    # plot_basic(Exp, 'Unc', show_gps=False)

    # plot_basic_box(Exp, kind='Rate')
    # plot_basic_box(Exp, 'Ashe Woodside'.split(), kind='Unc')

    # plot_hist(Exp)
    # plot_gps(Exp)

    plot_insets(Exp)
    # plot_insets(Exp, 'Ashe Woodside'.split())

    # plot_transect(Exp)


    # plot_cities(Exp, city='Queens', kind='Rate')
