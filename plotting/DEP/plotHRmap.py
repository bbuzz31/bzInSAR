...
CITIES_NYC    = 'Manhattan Brooklyn Queens Hudson Essex'.split() + \
                                        ['Staten Island', 'Bergen County']

CITIES_HR     = 'Norfolk Portsmouth Suffolk Hampton'.split() + \
...
class PlotHR(ExpBase):
    ...
    def _set_plot_parms(self):
        ...
        if self.kind == 'Rate':
            norm = mpl.colors.TwoSlopeNorm(0, -4, 4) if self.continuous else \
                mpl.colors.BoundaryNorm(np.arange(-4, 5, 1), 256)
    ...
    def plot_basic(self, axes=None, da=None):
        ...
        axes.add_image(StamenOrWhatever, zoom=13)
        ...
        m    = np.nanmedian(da)
        log.critical (f'InSAR Vup {self.kind}: {m:.2f} [{l:.2f}, {h:.2f}]')
    ...
    def plot_gps(self, axes=None):
        ...
        m2     = self.df_gps.u_vel.median()
        l2, h2 = self.df_gps.u_vel.quantile(0.003), self.df_gps.u_vel.quantile(0.997)
...
def plot_together(Exp1, show_gps=True, continuous=True):
    PObjR = PlotHR(Exp1, 'rate', show_gps, continuous)
    PObjU = PlotHR(Exp1, 'unc', show_gps, continuous)
...
def make_supp(Exp1):
    ...
    PObjU= PlotHR(Exp1, kind='unc')
    # PObjU.plot_gps()
    # PObjU.plot_basic_box(pts='Ashe Woodside'.split())

    PObjR = PlotHR(Exp1, kind='rate')
...
def make_numbers(Exp1):
    ...
    l, h = np.nanquantile(PObjR.da, 0.003), np.nanquantile(PObjR.da, 0.997)
    m    = np.nanmedian(PObjR.da)

    l1, h1 = np.nanquantile(PObjU.da, 0.003), np.nanquantile(PObjU.da, 0.997)
    ...
    l2, h2 = PObjR.df_gps.u_vel.quantile(0.003), PObjR.df_gps.u_vel.quantile(0.997)

    m3     = PObjR.df_gps.u_sig.median()
    l3, h3 = PObjR.df_gps.u_sig.quantile(0.003), PObjR.df_gps.u_sig.quantile(0.997)
    ...
    for city in CITIES:
        ...
        PObjR.plot_cities(city, loc, show=False)
        # PObjU.plot_cities(city, loc, show=False)
    return
...
if __name__ == '__main__':
    Exp0      = ExpBase(HR_SR, 'ERA5_SET_PM_ex_Fast', 'SPVA', neofs=25)
    # diagnostics(Exp0)
    make_main(Exp0)
    # make_supp(Exp0)
    # make_numbers(Exp0)
    ...
    path_figs = op.join(op.expanduser('~'), 'Desktop', 'VLM', 'HR_2023')
