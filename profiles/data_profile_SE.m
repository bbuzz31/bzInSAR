...
%% UPDATES BB - August, 2021
% share xaxis between rows
...
% figprop.n_crossplot               Number of panels (vertical) to plot in one figure
...
for k_profile=1:n_profiles
    ...
    %%% PLOTTING
    if counter==1
        % profile_fig = figure('position',[1 9 1424 726]);
        profile_fig = figure('position',[1 9 1424 726]);
    else
        try
            figure(profile_fig)
...
% % % % % % %     grid on
    ...
    if counter==n_crossplot
        counter=1;
        xlabel('Distance along crossection [km]','fontsize',fontsize)
    else
        xlabel('','fontsize',fontsize)
        set(hsubplot(counter),'Xticklabel',[]) % BZ - turn off xaxis ticklabels
        counter= counter+1;
    end
...
for k=1:length(hsubplot)
    xlimits = [min([get(hsubplot(k),'XLim') xlimits(1)]) max([get(hsubplot(k),'XLim')  xlimits(2)])];
end
for k=1:length(hsubplot)
    if isfield(figprop,'xlimits');
        if length(figprop.xlimits)==2;
            set(hsubplot(k),'XLim',figprop.xlimits)
        else
            set(hsubplot(k),'XLim',xlimits)
        end
    else
        set(hsubplot(k),'XLim',xlimits)
    end
