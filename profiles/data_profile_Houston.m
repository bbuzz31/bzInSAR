function [] = data_profile(data_or,data_std_or,lonlat_or,points_list,bins,R,figprop)
...
%     endfigprop.units
    ...
    left     = 0.115;
    bot      = 0.1;
    sp  = 0.30; % spacing in between rows
    xlimits21=figprop.xlimits(1)./2;
    % xlimits22=70; % hardcoded xlimits
    xlimits22=figprop.xlimits(2); % 

    if k_profile==1
        hsubplot(counter2) = subplot('Position',[left bot+2*sp spwidth/2.0 spheight]);
        set(hsubplot(counter2),'XLim', [xlimits21, xlimits22]);
        set(hsubplot(counter2),'xtick', 0:5:xlimits22);
    elseif k_profile == 2
        hsubplot(counter2) = subplot('Position',[left bot+1*sp spwidth/2.0 spheight]);
        set(hsubplot(counter2),'XLim', [xlimits21, xlimits22]);
        set(hsubplot(counter2),'xtick', 0:5:xlimits22);

    elseif k_profile == 3
        disp('profile 3')
        hsubplot(counter2) = subplot('Position',[left bot spwidth spheight]);
%         set(hsubplot(counter2),'XLim', [xlimits21, xlimits22]);
...
%         set(hsubplot(counter2),'xtick', 0:10:xlimits22);
    ...
    % Add the labels ----------------------------------------- BZ
    % disp(counter2)
    text(0.0, 0.99, char(counter2 + 64), 'Units', 'normalized', ...
                    'FontSize', 20, 'Color', 'w', 'BackgroundColor', 'k')
    text(0.96, 0.99, strcat(char(counter2 + 64),"'"), 'Units', 'normalized', ...
                    'FontSize',20, 'Color', 'w', 'BackgroundColor', 'k')
...
% % % % % % %     grid on
    ...
    ax = gca;
    ylabel(units,'fontsize',fontsize, 'rotation', 270)
    ...
    if counter==n_crossplot
        counter=1;
        xlh = xlabel('Distance along cross-section (km)','fontsize',fontsize);

    else
        if counter<=1
          set(hsubplot(counter),'Xticklabel',[]) % BZ -- turn off ticklabels
        end
   ...
   if ~isempty(AUXdata_keep)
       for k_errorbars = 1:size(XY_AUXdata_keep,2)
           ...
           if strcmpi(figprop.data_draw_manual,'n')
               hold on
               %one or double std error bar
               if figprop.sourceData_tsigma_alpha > 0
                herror2 = errorbar(XY_AUXdata_keep(1,k_errorbars),AUXdata_keep(k_errorbars,1),2*AUXdata_keep(k_errorbars,2));
               else
                herror2 = errorbar(XY_AUXdata_keep(1,k_errorbars),AUXdata_keep(k_errorbars,1),AUXdata_keep(k_errorbars,2));
               end
               set(herror2,'color',AUXdata_color{k_errorbars},'linewidth',2);
%                text(x_coordinates,[y_coordinates(1) y_coordinates(1)], 'hello');

               % ADD STATION LABELS -- fail
%                text(XY_AUXdata_keep(1,k_errorbars), AUXdata_keep(k_errorbars,1)+5, 'HELLO');
               ...
               % add the GPS stations and 2x the error; BZ use 1 sig if
               % 2sig alpha is 0
               if figprop.sourceData_tsigma_alpha > 0
                   scale = 2;
               else
               ...
               y_coordinates = [AUXdata_keep(k_errorbars,1) - scale*AUXdata_keep(k_errorbars,2)  AUXdata_keep(k_errorbars,1) + scale*AUXdata_keep(k_errorbars,2)];
...
% move xlabel to center and pad a bit if more than one transects
if exist('xlh','var') == 1
  xlh = xlh;

    xlh_pos=get(xlh,'position');
    xlh_pos(1)= xlh_pos(1); %BZ + 30;
    xlh_pos(2)= xlh_pos(2)-1; %- 2;
